...
print(f'[1/3] 상품 등록 완료 — product_no: {pno}')
...
r2 = requests.put(f'{BASE}/products/{pno}/seo', headers=headers, json=seo)
print(f'[2/3] SEO 설정 완료 — {r2.status_code}')

# 3. 네이버 홍보문구
r3 = requests.put(f'{BASE}/products/{pno}', headers=headers, json={
    'request': {'naverpay_product_name': '스노윅 실버 다이아 퍼퓸용기 30ml 분사형 DIY 공병'}
})
print(f'[3/3] 네이버 홍보문구 — {r3.status_code}')
